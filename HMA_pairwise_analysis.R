...
ggplot(pair_data, aes(x=ober, y=real))+
  ...
  xlab("Heteroduplex mobility distance (M)")+
  ylab("Nucleotide distance")+
  geom_point(size = 3.5, shape = 21, bg = 'gray')+
  annotate(label = sprintf("y = %.3f M + %.3f, R² = %.3f",lm.out$coefficients[2], lm.out$coefficients[1], summary(lm.out)$r.squared), geom = "text", x = 0.15, y = 0.18, size = 7)+
